...
bool StaticObjectsPerceptionModule::closeInitialization()
{
  ...
  if(config_file_ != "")
  {
    if(readConfiguration(config_file_) == false)
      return false;
  }
                                              ...
                                              const std::array<double,3>& scale)
{
  ...
  if(shape.type == SHAPE_MESH)
  {
    ...
    if(percepts_.find(obj.id()) == percepts_.end())
    {
      percepts_.emplace(obj.id(), obj);
      ShellDisplay::success("[StaticObjectsPerceptionModule] create shape for " + name);
    }
    ...
    {
      percepts_.at(obj.id()) = obj;
      ShellDisplay::success("[StaticObjectsPerceptionModule] update position for " + name);
    }
  }
  else
